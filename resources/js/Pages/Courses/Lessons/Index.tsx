...
const LessonIndex: React.FC<Props> = ({ courses = [] }) => {
    ...
    return (
                                                            ...
                                                            {lesson.materials && lesson.materials.length > 0 ? (
                                                                <ul>
                                                                    {lesson.materials.map((material) => (
                                                                        <li key={material.id} className="flex justify-between items-center mb-2 px-3 py-1 text-gray-200 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-800 dark:border-gray-600">
                                                                            <span className="flex-1 text-center">{material.title} ({material.type})</span>
                                                                            <div className="flex space-x-2">
                                                                                <button onClick={() => handleEditMaterialClick(lesson.id, material)} className="text-gray-900 bg-gradient-to-r from-lime-200 via-lime-400 to-lime-500 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-lime-300 dark:focus:ring-lime-800 shadow-lg shadow-lime-500/50 dark:shadow-lg dark:shadow-lime-800/80 font-medium rounded-lg text-sm px-1.5 py-1 text-center m-2 mb-2">
                                                                                    <i className="fas fa-edit"></i>
                                                                                </button>
                                                                                <button onClick={() => handleDeleteMaterialClick(lesson.id, material.id)} className="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 shadow-lg shadow-red-500/50 dark:shadow-lg dark:shadow-red-800/80 font-medium rounded-lg text-sm px-1.5 py-1 text-center m-2 mb-2">
        ...
        <div className='container mx-auto p-4 mt-4 rounded-lg'>
            <InertiaLink href="/materials/view" className="relative inline-flex items-center justify-center p-1.5 mb-2 me-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-cyan-500 to-blue-500 group-hover:from-cyan-500 group-hover:to-blue-500 hover:text-white dark:text-white focus:ring-4 focus:outline-none focus:ring-cyan-200 dark:focus:ring-cyan-800">Voir les Matériaux</InertiaLink>
